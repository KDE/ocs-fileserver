...
class Readepub {
    ...
    */
    ...
    private function _getTOC() {
        $tocFile = $this->getManifest('ncx');
        $tocContents = @simplexml_load_file($this->ebookDir.'/'.$this->opfDir.'/'.$tocFile['href']);
        if (!$tocContents) return false;

        $toc = array();
        foreach($tocContents->navMap->navPoint AS $navPoint) {
            $toc = $this -> _process_navPoint($toc, $navPoint);
        }
    ...
    /*
     * Recursive function to get all navPoints, which may be nested
     */
    private function _process_navPoint($toc, $navPoint) {
        $navPointData = $navPoint->attributes();
        $toc[(string)$navPointData['playOrder']]['id'] = (string)$navPointData['id'];
        $toc[(string)$navPointData['playOrder']]['name'] = (string)$navPoint->navLabel->text;
        $toc[(string)$navPointData['playOrder']]['src'] = (string)$navPoint->content->attributes();

        if ($navPoint->navPoint) {  // If there are more navPoints within the current one
            foreach ($navPoint->navPoint as $nested_navPoint) {  // Process them too
                $toc = $this -> _process_navPoint($toc, $nested_navPoint);
            }
        }
